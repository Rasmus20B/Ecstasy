...
export namespace util {
  namespace typelist {
    ...
    template<typename T, typename List>
    consteval auto containsT(List) -> bool {
      if constexpr(List::size == 0) {
        return false;
      } else if(std::is_same_v<Front<List>, T>) {
        return true;
      } else {
        return containsT<T>(PopFront<List>{});
      }
    }

    template<typename T, typename List>
    inline constexpr bool contains = containsT<T>(List{});

    using T1 = Typelist<int, size_t, float, char, double, std::string>;
    static_assert(contains<char, T1> == true);
    ...
    template<template<typename T> typename Pred, typename List>
    inline constexpr int count_frequency = count_frequencyT<Pred>(List{});

    template<typename T, typename List>
    inline constexpr int count_frequency_of_type = count_frequency_of_typeT<T>(List{});
